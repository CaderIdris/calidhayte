...
def prepare_datasets(
    ...
    uncal_sets = filter(
        lambda x: bool(re.search(r"^Uncalibrated ", x)), datasets_to_use
    )
    cal_sets = filter(
        lambda x: bool(re.search(r"^Calibrated ", x)), datasets_to_use
    )
    ...
    if pymc_bool:
        ...
        for cal_key in cal_sets:
            for pymc_subset in ["Mean"] + min_max_sets:
                selected_datasets[f"{cal_key} ({pymc_subset})"] = datasets[
